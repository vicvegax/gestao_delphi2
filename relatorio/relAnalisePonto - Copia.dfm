    ...
    object Memo1: TMemo
      ...
      Lines.Strings = (
        'select '
          ...
          '  row_number() over (order by uni.descr, eve.descr, mov.id) as l' +
          'inha,'
        '  uni.descr as ponto, eve.descr as evento, '
        '  if(mov.valor<0,mov.valor,0) as tot_debito, '
        '  if(mov.valor>0,mov.valor,0) as tot_credito,'
        
          '  sum(mov.valor) over (order by uni.descr, eve.descr, mov.id) as' +
          ' vl_total'
