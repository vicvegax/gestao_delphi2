object fConfig: TfConfig
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Configura'#231#245'es'
  ClientHeight = 506
  ClientWidth = 1077
  Color = clBtnFace
  Constraints.MinHeight = 471
  Constraints.MinWidth = 573
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poScreenCenter
  OnCreate = FormCreate
  TextHeight = 15
  object Label4: TLabel
    Left = 8
    Top = 432
    Width = 285
    Height = 15
    Caption = '* As altera'#231#245'es ser'#227'o aplicadas para todos os Usu'#225'rios!'
  end
  object imgLogo: TImage
    Left = 560
    Top = 78
    Width = 511
    Height = 346
    Proportional = True
    Stretch = True
    Transparent = True
    OnDblClick = imgLogoDblClick
  end
  object imgSelo: TImage
    Left = 561
    Top = 16
    Width = 219
    Height = 56
    Proportional = True
    Stretch = True
    OnDblClick = imgSeloDblClick
  end
  object Label10: TLabel
    Left = 8
    Top = 400
    Width = 208
    Height = 15
    Caption = '*** Dois Cliques para trocar as imagens!'
  end
  object btOK: TBitBtn
    Left = 112
    Top = 456
    Width = 129
    Height = 33
    Caption = '&Confirmar'
    Default = True
    Glyph.Data = {
      
      56B00E3226560000000600000002000000000000000000000000}
    TabOrder = 2
    OnClick = btOKClick
  end
  object btCancelar: TBitBtn
    Left = 328
    Top = 456
    Width = 129
    Height = 33
    Cancel = True
    Caption = '&Cancelar'
    Default = True
    Glyph.Data = {
      
      B5D040437D951212223000000004000000010000000000000000}
    ModalResult = 2
    TabOrder = 3
  end
  object GroupBox1: TGroupBox
    Left = 8
    Top = 16
    Width = 537
    Height = 201
    Caption = 'Defini'#231#227'o de Cores'
    TabOrder = 0
    object Label2: TLabel
      Left = 88
      Top = 64
      Width = 136
      Height = 15
      Caption = 'Movimento Em Conta (C)'
    end
    object Label3: TLabel
      Left = 88
      Top = 96
      Width = 135
      Height = 15
      Caption = 'Movimento Liquidado (L)'
    end
    object Label5: TLabel
      Left = 88
      Top = 128
      Width = 161
      Height = 15
      Caption = 'Linha de Documento Entregue'
    end
    object Label1: TLabel
      Left = 88
      Top = 32
      Width = 139
      Height = 15
      Caption = 'Movimento Agendado (A)'
    end
    object Label9: TLabel
      Left = 88
      Top = 160
      Width = 195
      Height = 15
      Caption = 'Data Vencimento ('#218'lt. Reg. Contrato)'
    end
    object cbAgendado: TdxColorEdit
      Left = 296
      Top = 24
      Properties.ImmediatePost = True
      Properties.ColorPalette = cpExtended
      Properties.ItemSize = 35
      TabOrder = 0
      Width = 121
    end
    object cbEmconta: TdxColorEdit
      Left = 296
      Top = 56
      Properties.ImmediatePost = True
      Properties.ColorPalette = cpExtended
      Properties.ItemSize = 35
      TabOrder = 1
      Width = 121
    end
    object cbLiquidado: TdxColorEdit
      Left = 296
      Top = 88
      Properties.ImmediatePost = True
      Properties.ColorPalette = cpExtended
      Properties.ItemSize = 35
      TabOrder = 2
      Width = 121
    end
    object cbEntregue: TdxColorEdit
      Left = 296
      Top = 120
      Properties.ImmediatePost = True
      Properties.ColorPalette = cpExtended
      Properties.ItemSize = 35
      TabOrder = 3
      Width = 121
    end
    object cbUltMov: TdxColorEdit
      Left = 296
      Top = 152
      Properties.ImmediatePost = True
      Properties.ColorPalette = cpExtended
      Properties.ItemSize = 35
      TabOrder = 4
      Width = 121
    end
  end
  object GroupBox2: TGroupBox
    Left = 8
    Top = 224
    Width = 537
    Height = 161
    Caption = 'Links para Pol'#237'ticas de'
    TabOrder = 1
    object Label6: TLabel
      Left = 8
      Top = 66
      Width = 135
      Height = 15
      Caption = 'Seguran'#231'a da Informa'#231#227'o'
    end
    object Label7: TLabel
      Left = 8
      Top = 34
      Width = 101
      Height = 15
      Caption = 'Privacidade Interna'
    end
    object Label8: TLabel
      Left = 8
      Top = 98
      Width = 37
      Height = 15
      Caption = 'Senhas'
    end
    object Label11: TLabel
      Left = 8
      Top = 130
      Width = 101
      Height = 15
      Caption = 'Adequa'#231#227'o '#224' LGPD'
    end
    object edLINK1: TEdit
      Left = 152
      Top = 24
      Width = 377
      Height = 23
      TabOrder = 0
    end
    object edLINK2: TEdit
      Left = 152
      Top = 56
      Width = 377
      Height = 23
      TabOrder = 1
    end
    object edLINK3: TEdit
      Left = 152
      Top = 88
      Width = 377
      Height = 23
      TabOrder = 2
    end
    object edLINK4: TEdit
      Left = 152
      Top = 120
      Width = 377
      Height = 23
      TabOrder = 3
    end
  end
  object dlgImg: TOpenPictureDialog
    Title = 'Abrir Imagem'
    Left = 432
    Top = 384
  end
end
